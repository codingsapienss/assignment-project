(()=>{
    ...
    let camera__screen, video, shutter_btn, retake_btn, close_btn, name_input, button , save_btn;
    ...
    function saveData () {
      closeCameraPage()
            if ( name_input.value.trim() != '' && /^[A-Za-z]{1,30}$/.test(name_input.value)) {
               localStorage.setItem('name' ,name_input.value )
            } else {
                alert("Please enter a valid name containing only letters and not exceeding 30 characters.");
            }
